...
function ArtistProfile({ artist, user, currentSong, isPlaying, onTogglePlay, onPlaySong, onAlbumClick, onBack, onAddToPlaylist }) {
    ...
    useEffect(() => {
        if (artist && artist.id) {
            fetch(`http://localhost:8080/api/artists/${artist.id}`)
                .then(res => res.json())
                .then(data => {
                    setFullArtist(data);
            ...
            fetch('http://localhost:8080/api/songs')
                .then(res => res.json())
                .then(data => {
                    const artistSongs = data.filter(s => s.album?.artist?.id === artist.id);
                    ...
                    {topSongs.map((song, index) => {
                        ...
                        return (
                            <div key={song.id} className="song-row" onDoubleClick={() => onPlaySong(song)} style={{ display: 'flex', alignItems: 'center', padding: '10px 16px', borderRadius: '4px', cursor: 'default', background: isCurrent ? '#2a2a2a' : 'transparent', transition: 'background 0.2s' }}>
                                ...
                                <img src={song.album?.id ? `http://localhost:8080/api/albums/${song.album.id}/cover` : null} style={{ width: '40px', height: '40px', objectFit: 'cover', borderRadius: '4px', marginRight: '16px', marginLeft: '10px', background: '#333' }} onError={(e) => e.target.style.display='none'} />
                                <div style={{ flex: 1, display:'flex', flexDirection:'column' }}>
                                    <span style={{ fontWeight: '500', color: isCurrent ? '#1DB954' : 'white' }}>{song.title}</span>
                                    <span style={{ fontSize: '12px', color: '#b3b3b3' }}>{song.genre}</span>
                                </div>
