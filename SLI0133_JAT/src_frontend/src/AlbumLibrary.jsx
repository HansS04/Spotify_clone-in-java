...
function AlbumLibrary({ user, onAlbumClick }) {
    ...
    useEffect(() => {
        if (user) {
            let url = 'http://localhost:8080/api/albums'; // Default: Všechna alba (pro posluchače)

            if (user.role === 'ARTIST') {
                // Pokud je umělec, chce vidět jen SVÁ alba
            ...
            fetch(url)
                ...
                .then(data => {
                    // Pojistka: pokud data nejsou pole, nastavíme prázdné pole
                ...
                .catch(err => {
                    console.error(err);
                    setAlbums([]); // Při chybě vyčistíme, aby to nepadalo
                });
        }
    }, [user]);
