    ...
    useEffect(() => {
        if (album && album.id) {
            fetch('http://localhost:8080/api/songs')
                .then(res => res.json())
                .then(data => {
                    // SongDTO uses albumId (not nested album object)
                    const albumSongs = data.filter(song => song.albumId && String(song.albumId) === String(album.id));
                    setSongs(albumSongs);
                })
                    ...
                    return (
                        <div key={song.id} className="song-row" onDoubleClick={() => onPlaySong(song)} style={{ display: 'flex', alignItems: 'center', padding: '10px 0', color: isCurrent ? '#1DB954' : '#b3b3b3', borderBottom: '1px solid rgba(255,255,255,0.05)', cursor: 'default' }}>
                            ...
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ fontSize: '13px', color: '#b3b3b3' }}>{song.artistName}</div>
                            </div>
